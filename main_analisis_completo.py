...
CONSTANTS = {
    ...
    # --- VELOCIDADES DE TRANSICIÓN (en m/s) ---
    'v_stationary_threshold': 1.0,      # < 3.6 km/h (prácticamente detenido)
    'v_slow_threshold': 5.0,            # < 18 km/h (velocidad muy baja)
    
    # --- TIEMPOS DE MANIOBRA DINÁMICOS (segundos) ---
    't_maniobra_stationary': 15,        # Tiempo de cambio con vehículo detenido (v < 1.0 m/s)
    't_maniobra_slow': 45,              # Tiempo de cambio con velocidad baja (1.0 <= v < 5.0 m/s)
    ...
    # --- UMBRALES DE ZONA GRIS (Incertidumbre) ---
    'gray_zone_high': 0.73,          # Umbral alto en zona gris (dist > 0.73 con física posible = alerta)
    'gray_zone_medium': 0.63,        # Umbral medio en zona gris (dist > 0.63 con física posible = sospecha)
    'gray_zone_medium_high': 0.78,   # Umbral medio-alto en zona gris (dist > 0.78 con física dudosa = revisar)
    
    # --- GESTIÓN DE IDs ---
    'id_reuse_match_threshold': 0.7, # Umbral relajado para reutilizar IDs (P95 intra-conductor)
    'mini_id_frame_threshold': 2,     # Máximo de frames para considerar un ID como "mini" (probable falso positivo)
    ...
    # --- REPORTES Y SALIDA ---
    'max_mini_ids_to_show': 30,  # Máximo de mini-IDs a mostrar en el reporte (evita spam)
}
...
def process_asset_group(df_asset, asset_name="Unknown"):
    ...
    for i in range(1, len(df_clean)):
        prev = df_clean.iloc[i-1]  # Frame anterior
        cur = df_clean.iloc[i]      # Frame actual
        ...
        # el cambio es más rápido. Si va a alta velocidad, necesita más tiempo.
        # Usamos la velocidad máxima entre ambos frames como referencia
        v_max = max(v_prev, v_curr)
        v_stationary = CONSTANTS['v_stationary_threshold']
        v_slow = CONSTANTS['v_slow_threshold']
        t_man_stationary = CONSTANTS['t_maniobra_stationary']
        t_man_slow = CONSTANTS['t_maniobra_slow']
        
        if v_max < v_stationary:  # Vehículo prácticamente detenido
            t_maniobra_dinamico = t_man_stationary
        elif v_max < v_slow:  # Velocidad muy baja
            t_maniobra_dinamico = t_man_slow
        ...
        if main_distance is None:
            ...
            if main_distance < visual_match:
                # Validar contra anomalías (saltos bruscos de velocidad)
                if is_sudden:
            ...
            # Embeddings en zona intermedia (0.5 <= dist <= 1.0)
                ...
                gray_high = CONSTANTS['gray_zone_high']
                gray_medium = CONSTANTS['gray_zone_medium']
                gray_med_high = CONSTANTS['gray_zone_medium_high']
                ...
                if P_fisica > CONSTANTS['phys_possible']:
                    ...
                    elif main_distance > gray_medium:
                    ...
                    if main_distance > gray_med_high:
...
def main():
    ...
    # Detectar mini-IDs (≤ mini_id_frame_threshold frames)
    mini_id_threshold = CONSTANTS['mini_id_frame_threshold']
    persona_frame_counts = out_df.groupby(['ASSET_ID', 'PERSONA_ID']).size()
    mini_ids = persona_frame_counts[persona_frame_counts <= mini_id_threshold]
    ...
    # 1. Detectar IDs mini (≤ mini_id_frame_threshold frames)
    mini_id_threshold = CONSTANTS['mini_id_frame_threshold']
    mini_ids_by_asset = {}
    for asset_id in out_df['ASSET_ID'].unique():
        asset_df = out_df[out_df['ASSET_ID'] == asset_id]
        ...
        mini_ids = {pid: count for pid, count in persona_frame_counts.items() if count <= mini_id_threshold}
        if mini_ids:
    ...
    if mini_ids_by_asset:
        ...
        count_shown = 0
        max_show = CONSTANTS['max_mini_ids_to_show']
