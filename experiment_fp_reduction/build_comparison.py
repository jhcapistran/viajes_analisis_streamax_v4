...
for name in ["random_04", "reviewed_07"]:
    ...
    base_detail = pd.read_csv(os.path.join(EXP_DIR, f"baseline_detail_{name}.csv"))
    mem_detail = pd.read_csv(os.path.join(EXP_DIR, f"memory_detail_{name}.csv"))
...
df.to_csv(os.path.join(EXP_DIR, "comparison_summary.csv"), index=False)
